...
| IPC/TUI 健壮性（REQ-IPC-0001） | ✅ | 2026-08-07 test/ipc.test.ts 13 条：streaming FrameDecoder 多字节切分逐字节一致、同秒消息复合游标翻页无丢无重、队列超限/写失败踢连接且内存有界、socket 600、limit 夹取、ANSI/OSC 52 注入 strip |
...
| TUI/UI（REQ-UI-0001/2/3） | ✅ | 2026-08-07 test/ipc.test.ts：hello filter 服务端过滤 snapshot/history/broadcast/usage + stats 聚合与 DB 一致（lastId 防双计）；真实 attach 冒烟：attach A 单 bot 视角（仅小雪 LOCAL）、全局面板双 bot 遥测（ep/last/cum/$/hit 89.4%）、非法 id 报错列出清单；kitty Image 渲染（pi-tui 原生，非 kitty 自动降级）；面板行截断 |
