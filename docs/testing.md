...
| Pi auth/settings/shared runtime（REQ-PLAT-0002 T13j1/T13j2） | ✅ runtime boundary / ⏳ onboarding + vision | 2026-08-08 canonical schema/examples/`.env.example`不再含provider key；legacy字段引用不存在env仍被丢弃。真实SettingsManager fixture覆盖global/project defaults与missing/invalid；隔离auth fixture覆盖API-key/OAuth；N bot fake锁一次runtime、0次credential injection与same/cross-provider选择，provider/compaction错误只落固定category。config/runtime/daemon/flush/Tg targeted 99 pass / 979 assertions；本机A/B只读preflight均为`deepseek/deepseek-v4-flash:medium` + stored auth。 |
...
| Pi native `/tg config`（REQ-ONBOARD-0001 T13d） | ✅ fake host / ⏳ real Telegram credential smoke | 2026-08-08 extension+core 52 pass / 627 assertions：loader失败仍有help/completion/dispatch；Pi原生select/input/confirm/editor完整首配，11个fresh dialog取消点零目标文件，existing editor取消逐字节不变；`.env bots_config`自定义/缺失source不被旁路覆盖；fake controlled restart仅在明确ready后自动all-bots attach，失败保留production-loader可读配置、不给connected假成功，并脱敏token/key且提示status/retry。typecheck通过。 |
...
| 长运行 smoke | ⏳ Phase 9 | - |
| 当前全量回归 | ✅ | 2026-08-08 `bun test`：352 pass / 0 fail / 4732 assertions；`bun run check` 通过；cache v5 golden 6/6 |
