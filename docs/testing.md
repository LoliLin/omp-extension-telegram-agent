...
| Telegram群内控制（REQ-CMD-0001） | ✅ deterministic full chain | 2026-08-08 45 targeted / 374 assertions以fake Poller→control→目标BotApi→reply_parameters→canonical→IPC锁一次执行/创建/广播、权限与mutation queue；T14不在真实deployment重复执行会改变routing/compact状态的管理员命令。 |
...
| Direct reply provider delivery（REQ-REPLY-0001 T10o） | ✅ unit + real aggregate trace | 2026-08-08 70 targeted / 2680 assertions覆盖 transaction、priority、busy/restart与A/B隔离；T14 只读聚合显示 bot-1 obligation 7/7、bot-2 19/19 均已创建/交付，当前 pending=0，且两只 bot 均有真实 reply send event。 |
...
| 当前全量回归 | ✅ | 2026-08-08 `bun test`：424 pass / 0 fail / 5059 assertions（全局外网guard生效）；`bun run check`通过；cache v7 golden 6/6；mdBook 18 Markdown/98 links、21 HTML/620 links通过。 |
