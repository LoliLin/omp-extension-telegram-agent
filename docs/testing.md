...
| 双语用户与机器文档（DOC-0001 / ONBOARD T13e） | ✅ source / ⏳ mdBook CI | 2026-08-08 中英README首屏均含语言/guide入口与三步首配；两本guide各7个source文件覆盖install→Telegram/provider→config→Pi→ops→troubleshooting与六项cost design；AGENTS一跳到maintainer guide及development/traceability/testing/cache/documentation/release。77个Markdown本地link target全通过，产品主体无小雪/小雨/`bots[0]`，config+extension 70 pass / 652 assertions与typecheck通过。正式mdBook build/generated-link/Pages gate留T13f。 |
