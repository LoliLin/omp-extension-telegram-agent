...
| Pi editor compose（REQ-UI-0005 T6/T13o） | ✅ unit / ⏳ real smoke | 2026-08-08 extension/engine/IPC/manual/cache共100 pass / 1156 assertions：filtered attach直发、全局A/B逐条Pi原生select、单bot零select、取消/selector异常逐字节恢复、sticky/scope/off、RPC/extension continue、附件/空文本、选择/发送in-flight防重、detach迟到选择失效、ACK unknown/no retry、footer双状态与全lifecycle；`bun run check`通过，cache v6不变。真实Pi/Telegram留T14。 |
...
| Markdown→Telegram entities（REQ-TG-0004 T13p） | ✅ offline / ⏳ real smoke | 2026-08-08 converter/API/runtime覆盖plain零entity、CJK/emoji UTF-16、nested style/code分割、heading/list/task/table/quote/fence、安全public link、inert image/raw HTML、4096/100-entity界限与固定脱敏错误；classic `sendMessage`精确payload、entity 400单次plain fallback、timeout/429/5xx/non-JSON/network/persistence零重发、组合sticker/reply、`markdown_sent`event和cache schema v7均锁定。自动测试由全局guard保证0真实Telegram/TinyFish调用；真实plain/bold/list/code/reply留T14。 |
...
| 长运行 smoke | ⏳ Phase 9 | - |
| 当前全量回归 | ✅ | 2026-08-08 `bun test`：424 pass / 0 fail / 5059 assertions（全局外网guard生效）；`bun run check`通过；cache v7 golden 6/6。双mdBook最终重建留T14。 |
