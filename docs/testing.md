...
| portable Pi launcher（REQ-ONBOARD-0001 T13a） | ✅ bootstrap / ⏳ `/tg config` | 2026-08-08 四个 Pi package 精确锁定 registry 0.84.1、lock 无 file dependency；launcher fake fresh/installed/failure 共4 tests，项目与无 `.env`/config/persona/node_modules/sibling Pi 的隔离目录均以 `bun run pi --version` 输出0.84.1。完整无配置 `/tg config` 启动留 T13d/T14。 |
| atomic onboarding config（REQ-ONBOARD-0001 T13c） | ✅ core / ⏳ native wizard | 2026-08-08 `test/onboarding-config.test.ts` 7 cases覆盖fresh三文件+final loader/0600、existing deny、peer/token/provider/model/persona validation、第二次rename失败全rollback、confirmed全量backup+env merge、existing validate/editor exact round-trip+confirmed replace及duplicate env key。events仅phase/相对path；secret不进config/event/error。search=false首配不要求TinyFish key。 |
