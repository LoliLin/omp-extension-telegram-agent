...
- vision 按 identity 持久化并跨 bot 复用。完整静态文件先写 0600 临时文件，再同目录 rename；图片 bytes 与绝对 path 不进 SQLite，`local_path`只保存当前`data/media`内的 cache-relative basename。daemon 启动时会按 basename 迁移旧绝对值，目标不存在则清空并允许有界后台队列重新获取。
