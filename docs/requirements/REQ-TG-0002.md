...
- **Status:** Approved（2026-08-08 已核对 Telegram Bot API，未实现）
- **Priority:** P1
...
Telegram Bot API 官方 [`sendChatAction`](https://core.telegram.org/bots/api#sendchataction) 规定：状态保持 5 秒或更短，bot 消息到达时客户端清除状态；长操作需要调用方续约。当前动作集合包含 `typing`、`choose_sticker` 等，但没有独立的 `thinking`。因此首版用 `typing` 表示通用“正在处理”，以 4 秒周期留出到期余量。
...
- 不伪造不存在的 Telegram `thinking` action，也不发送“正在思考”文本消息。
...
- **R2 — 官方动作：** 使用当前 bot token 调 `sendChatAction {chat_id, action:"typing"}`。chat id 只取 deployment 的配置群，不接收模型或群消息参数；API 方法归属 `src/telegram/`。
...
无。未来若要根据确定的 sticker send 切换 `choose_sticker`，另开 UX 需求并证明不会增加状态抖动；本需求不预留 generic action 输入。

## 追溯

- Plans: `PLAN-20260808-complete-new-reqs#T10g`、`#T10i`
