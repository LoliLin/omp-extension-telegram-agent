...
- **Status:** Done（2026-08-08；真实 Pi TTY 已显示一级菜单与动态 bot 二级菜单）
- **Priority:** P2
...
- config load 失败只返回静态一级项且不抛错；targeted extension/cache 28 tests 与 typecheck 通过。T14 隐藏原文的真实 Pi TTY smoke 确认一级菜单与 `attach` 动态 bot 二级菜单都出现。
