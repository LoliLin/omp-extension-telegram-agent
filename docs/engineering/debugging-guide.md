...
4. 视频先检查`video_transcoder`；`video_transcoder_unavailable`可在安装后重试，`video_probe_failed`/`video_frame_extraction_failed`证明失败发生在provider前。不得记录命令stderr或path。
5. `agent_events.kind=vision`的固定`outcome`、`frames`与`providerCalled`证明foreground识别结果；跨bot路由时应使用任一已配置且有mapping的接收bot，`file_id_unavailable`只表示所有可用source均缺失。
