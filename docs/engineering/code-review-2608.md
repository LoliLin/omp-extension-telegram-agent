...
| P1 | Pi footer 通过 `as never` 伪造一份不完整 `AgentSession` 再传给 `FooterComponent` | 依赖组件内部读取形状，是升级脆弱的真实 hack；`/tg status` 已提供同一信息 | 删除 `/tg panel` 和 fake session adapter；保留按需的 `/tg status` 与 Telegram `/status` |
