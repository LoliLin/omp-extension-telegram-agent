...
- 用户随时在项目目录运行 `bun run pi`，用 `/tg attach [bot-id]` 在 Pi 原生 transcript 查看完整群聊、LOCAL 事件，并在 Pi 原生 footer status 查看数据库 telemetry 保留期的紧凑 lifetime usage；关闭 Pi 不影响 daemon，累计值不归零
