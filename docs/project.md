...
- 安装 `omp install <repo>` 后，扩展随 omp 自动加载；配置不存在时 `/tg config` 仍可用，并用 omp 原生 dialogs 建立 typed config、private `.env` 与 persona。
...
- 用户随时在 omp 里用 `/tg attach [bot-id]` 在 omp 原生 transcript 查看完整群聊、LOCAL 事件，并在与 omp 原生同构的 attached footer 查看数据库 telemetry 保留期 usage；此时隐藏无关的 omp operator usage 行，detach 后恢复；关闭 omp 不影响 daemon，累计值不归零
- `/tg more` 加载更早历史，`/tg detach` 断开实时订阅，`/tg status [bot]` 按需查看 Telegram usage
...
- 当前 compaction threshold = Pi估算32K tokens；成功后按 `compaction_keep_recent` token 预算保留近期原文（缺省 1 token 实际只剩摘要，生产推荐 20,000），更早内容仅保留摘要
