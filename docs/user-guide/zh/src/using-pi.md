...
Telegram feed 是一个 TUI-only Pi custom entry。滚动、resize、选择、主题、图片布局和 footer 都由 Pi 原生组件负责；插件在 editor 上方显示一行 feed scope，并通过 Pi 原生 footer status 显示紧凑 Telegram usage。消息不会因为展示而进入当前 Pi agent 的 provider context。
