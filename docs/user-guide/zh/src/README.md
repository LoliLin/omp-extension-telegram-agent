...
本指南面向部署和使用者。你不需要先理解内部架构，就能把 1..N 个可配置 AI 群友接入一个 Telegram supergroup，并用 omp 原生界面观察和操作。

## 最短路径

1. 准备群 ID 与 BotFather token，并用 omp `/login`、`/model`完成模型登录与默认选择。
2. 在仓库中运行 `omp 启动（`omp install <repo>` 后直接使用 `/tg` 命令）`。
3. 在 omp 执行 `/tg config`，完成后等待 all-bots feed 自动打开。

按顺序阅读：

- [安装与首次配置](getting-started.md)：Telegram与Pi模型准备、原生向导。
- [配置与添加 bot](configuration.md)：typed config、secret 边界、routing 和 N-bot。
- [在 omp 中聊天和观察](using-pi.md)：attach、compose、history 与 telemetry。
...
- 关闭 omp 不会停止 daemon；Telegram 群才是实际聊天场所，omp 是本机观察和控制界面。
