...
这样减少的是整个无意义调用，而不是在调用后省几个token。权威行为见[架构的 Routing 章节](../../../architecture.md)。

## 2. Stable provider prefix 复用cache

persona、固定协议和固定顺序tool schema构成稳定prefix。新消息只追加suffix，代码永不为加入动态信息而改写已经存在的prefix。

cache-visible grammar变化必须升级schema并开启新context epoch；UI、telemetry和operator命令不能偷偷改变provider bytes。权威规则见[Cache工程](../../../cache.md)。
...
因此打开Pi、滚动历史、切换panel或查看usage不会消耗一次聊天模型调用。权威边界见[Pi原生transcript架构](../../../architecture.md)与[Cache工程](../../../cache.md)。
...
4. 任何prompt/tool/serialization改动先按[开发指南的cache流程](../../../engineering/development-guide.md)验证golden和epoch。
