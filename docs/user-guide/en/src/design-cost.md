...
This avoids an entire unnecessary call instead of shaving a few tokens after starting one. See [Routing architecture](../../../architecture.md).
...
A cache-visible grammar change must increment the schema and open a new context epoch. UI, telemetry, and operator commands may not alter provider bytes. See [Cache engineering](../../../cache.md).
...
This separates the complete local source of truth from the context necessary for one run. See [Architecture](../../../architecture.md) and the [data model](../../../data-model.md).
...
Compaction itself uses an auxiliary model, so it is not an every-turn online optimizer. Configuration controls the threshold and retained amount; telemetry validates the result. See [Cache engineering](../../../cache.md) and [test status](../../../testing.md).
...
See the [Vision architecture](../../../architecture.md).
...
Opening Pi, scrolling history, changing a panel, or viewing usage therefore does not create a chat-model call. See the [Pi-native transcript architecture](../../../architecture.md) and [Cache engineering](../../../cache.md).
...
4. Before changing prompts, tools, or serialization, follow the cache process in the [development guide](../../../engineering/development-guide.md).
