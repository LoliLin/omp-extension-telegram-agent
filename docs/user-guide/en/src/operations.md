...
`/status` uses a Telegram rich message to show each bot's runtime state, provider/model/effective reasoning, current context/window/percentage, system/tool/summary/message/free segments as one red/purple/brown/blue/green square per rounded 1,024 tokens, average tok/s/send/think time, latest conversation request, retained SQLite totals, cache hit rate, latency/cost, routing, and latest compaction, with grouped thousands for statistics. It shares the [unified telemetry semantics](https://github.com/mizorewww/pi-extension-telegram-agent/blob/main/docs/telemetry.md) with Pi `/tg status`: detailed status fields come from one shared projection, `CH = R / (↑ + R + W)`, no cache read/write sample displays `—`, and current context never uses historical prompt totals. If Telegram definitively rejects the rich-message method or format before creating a message, the daemon sends one independently generated plain-text projection instead. It does not resend after timeouts, rate limits, server failures, or other uncertain outcomes, preventing duplicate replies.
