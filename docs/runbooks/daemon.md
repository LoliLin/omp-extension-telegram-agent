...
- attached feed 的两行footer保留cwd/branch/session name，并持续显示Telegram lifetime `↑/↓/R/W/CH/$`、latest context与model；无关的Pi operator usage行在attached期间隐藏，detach后恢复。`/tg status [bot]`列出runs/since/epoch、latest cache/output/reasoning/latency/cost与lifetime totals/平均latency。
