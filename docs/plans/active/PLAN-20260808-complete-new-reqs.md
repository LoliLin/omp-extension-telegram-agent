...
- **Status:** Active
- **Requirements:** REQ-STICKER-0002, REQ-ROUTE-0001, REQ-SEND-0001, REQ-CMD-0001, REQ-TG-0002, REQ-UI-0005, REQ-UI-0006, REQ-UI-0007, REQ-UI-0008, REQ-UI-0009, REQ-UI-0010, REQ-PLAT-0001, REQ-DOC-0001
...
- [ ] **T10h** — 增加有界 ephemeral assistant stream IPC、原位 Pi native card 与每次 feed change 的 host render request；validates: UI-0010 AC1–AC7；commit: native streaming behavior
- [ ] **T10i** — 增加每 bot `typing` activity lease、4 秒续约与 send/settle/shutdown 清理；validates: TG-0002 AC1–AC7；commit: Telegram response feedback
- [ ] **T10j** — 增加 Telegram `/tg` deterministic command service、public status、admin allowlist、持久 routing/cooldown override 与安全 manual compact；validates: CMD-0001 AC1–AC8；commit: Telegram control plane
...
- T10f/T10h: **NONE**；只消费已有 provider response stream 并更新 IPC/TUI 内存；不持久 partial、不新增模型调用或 provider-visible bytes。
- T10g/T10i: **NONE**；Telegram chat action 是每 active bot 每 4 秒至多一次的 side channel，LLM token/provider payload 不变。
- T10j: **NONE**；Telegram control command 与回复均由 deterministic control plane 消费，不进入 provider context；只有显式 compact 使用既有 summary 调用。
