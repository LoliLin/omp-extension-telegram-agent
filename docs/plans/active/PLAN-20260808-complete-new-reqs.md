...
- [x] **T13j3** — 删除向导provider/key输入并收口canonical examples、env与双语运维文档；validates: PLAT-0002 R4/R5/AC4/AC5；commit: `c95c695`
- [x] **T13k1a** — 删除Codex CLI子进程，用shared Pi Luna low执行视觉、转换静态格式并做启动预检；validates: VISION-0001 AC7 + PLAT-0002 R6/AC6；commit: `f4ff63b`
- [x] **T13k1b** — 将动态media gate改为两路有界并发并锁定一次序列化、catalog snapshot与脱敏遥测；validates: VISION-0001 AC1–AC8；commit: `6efd768`
...
- [ ] **T13k2** — 用T13k1a–T13k1c真实commit hash记录VISION-0001完成并在REQ-LIST勾选；validates: VISION-0001 traceability；commit: completion record
