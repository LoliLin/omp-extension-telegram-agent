...
`ffprobe` 与 `ffmpeg` 通过无 shell 的 argv 调用；frame 写入 0600 临时目录并在请求结束后删除。所有帧在**一次** Pi vision call 中发送，结果继续按 `file_unique_id` 持久化并跨 bot singleflight。整条下载→抽帧→provider流水线在Telegram下载前取得deployment-wide scheduler slot，所有bot共用`vision.concurrency`，避免各runtime分别放大本机FFmpeg负载。日志和 telemetry 只记录 kind、帧数、字节 bucket、耗时、usage、cost 与固定 outcome，不记录 identity、路径、命令 stderr、prompt 或描述正文。

主机缺少任一工具时，视频 MUST 在Telegram下载、scheduler预算与provider调用之前返回确定性fallback；daemon仍可ready，聊天、图片vision和sticker发送仍可用。`start`/`restart`/`status`给operator用途与安装提示，startup log 与 `bun run debug` MUST 给出固定的`video_transcoder_unavailable` impact/action，但不得向群内发告警。这个准备失败不得写成永久terminal vision cache，安装工具并restart后可重试。
...
视频视觉复用既有 append-only `media_update` grammar，不改写已存在 prefix。每个新视频最多新增一次 vision provider call，最多三张经过尺寸限制的 JPEG 共用该调用；persistent cache hit、跨 bot concurrent hit、缺transcoder与TUI展示均不增加provider调用或主对话token。缺transcoder还在下载与budget reservation前no-op。TGS发送与媒体回收新增provider调用为0。
...
5. `ffmpeg` / `ffprobe` 缺失时daemon、聊天、static image vision与sticker发送正常；不下载视频、不占budget/token，CLI/debug明确报告用途、影响与安装动作。
