...
- 固定hidden extension顺序为`tg-context → tg-compaction → tg-cache-observer → tg-assistant-persistence`。shared protocol是system prompt首段，persona随后，identity + format sticker catalog block 在末尾。主模型有效context window固定为64K；Pi估算32K时压缩，并只保留最后一个完整turn原文。
