...
- 固定hidden extension顺序为`tg-context → tg-compaction → tg-cache-observer → tg-assistant-persistence`。shared protocol是system prompt首段，persona随后，identity + format sticker catalog block 在末尾。主模型有效context window固定为64K；compaction 触发点 = 64K − max(16,384, 64K − compaction_threshold)，threshold 超过 49,152 会被配置校验拒绝；压缩后保留最近 compaction_keep_recent token 原文（缺省只保留最后一个完整turn）。
