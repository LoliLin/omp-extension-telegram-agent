...
2026-08-08：PLAT-0002 T13j1/T13j2已取消项目provider key、合并Pi defaults并建立单shared runtime；下一步T13j3删除向导provider/key流程并收口双语文档，之后依次vision、photo readiness、TinyFish fetch与T14总验收。PLAT-0002尚未勾选。
...
26. **进行中 `REQ-PLAT-0002`**：T13j1 `0859490`移除`BotConfig` provider secret/env与注入；T13j2从Pi global/project settings解析默认模型/thinking，并在pid lock后、Telegram前创建一次shared runtime预检N bot。当前本机只读preflight为A/B同一DeepSeek stored auth；向导、用户文档及视觉复用仍由T13j3/T13k1完成。
